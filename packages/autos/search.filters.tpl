	...
	<div class="form-group">
		...
		<select name="mk" class="form-control no-js js-car-make">
			...
			{foreach $car_blocks_data.filters.makes as $entry}
				<option value="{$entry.id}"{if isset($filters.params.mk) && $filters.params.mk == $entry.id} selected{/if}>{$entry.name|escape:'html'}</option>
			{/foreach}
		</select>
	</div>
	<div class="form-group">
...
{if isset($car_blocks_data.filters.services)}
	<div class="form-group">
		<label>{lang key='keywords'}</label>
		<input type="text" name="kw" placeholder="{lang key='services_keywords_placeholder'}" class="form-control"{if isset($filters.params.kw)} value="{$filters.params.kw|escape:'html'}"{/if}>
	</div>
	<div class="form-group">
		<label>{lang key='category'}</label>
		<select class="form-control" name="cat">
