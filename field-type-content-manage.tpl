...
{if isset($item.$fieldName)}
	...
	{elseif in_array($type, array(iaField::IMAGE, iaField::PICTURES, iaField::STORAGE)) && !in_array($item.item, array('autos', 'autos_services', 'autos_parts'))}
...
<div class="form-group{if iaField::TEXTAREA == $type} form-group--textarea{/if} {$field.class} {$field.relation}{if $field.for_plan && !$field.required} form-group--plan" style="display:none;{/if}" id="{$fieldName}_fieldzone">
	...
	{switch $type}
		...
		{case iaField::TEXTAREA break}
			{if !$field.use_editor}
				<textarea class="form-control" name="{$fieldName}" rows="5" id="{$name}">{$value|escape:'html'}</textarea>
				{if $field.length > 0}
					{ia_add_js}
$(function()
{
			...
			{if $value}
					<div class="row upload-items" id="{$fieldName}_upload_list">
						{foreach $value as $entry}
							<div class="{if 'autos_parts' == $item.item || 'autos_services' == $item.item}col-sm-3{else}col-sm-2{/if}">
								<div class="thumbnail upload-items__item">
									<div class="thumbnail__actions">
										<div class="btn-group">
											<span class="btn btn-default btn-xs drag-handle"><span class="fa fa-arrows"></span></span>
											<button class="btn btn-xs btn-danger js-delete-file" data-item="{$field.item}" data-field="{$fieldName}" data-item-id="{$item.id|default:''}" data-picture-path="{$entry.path}" title="{lang key='delete'}"><span class="fa fa-times"></span></button>
										</div>
									</div>
